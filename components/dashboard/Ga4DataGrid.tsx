...
import { Ga4ApiError, Ga4ApiService, Ga4DataRow } from "@/src/services/ga4Service"
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (err instanceof Ga4ApiError && err.code === "GA4_REPORT_NOT_WAREHOUSED") {
          setError("This Analytics breakdown is not available from stored GA4 data for the selected view yet.")
        } else if (err.message === "UNAUTHORIZED") {
          setError("Your session expired. Sign in again to load stored Analytics data.")
        } else if (err.message === "Failed to fetch") {
          setError("Network error: Unable to load the stored Analytics report right now.")
