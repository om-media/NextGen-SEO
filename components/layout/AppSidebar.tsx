...
export function AppSidebar({ selectedSite }: { selectedSite?: string }) {
  ...
  const limitMap = { free: 1, pro: 3, enterprise: 'Unlimited' }
  const tierLimit = userProfile?.tier ? limitMap[userProfile.tier] : 1
  const stringifiedLimit = tierLimit === 'Unlimited' ? '∞' : tierLimit
  ...
  return (
    <Sidebar>
      ...
      <SidebarFooter className="p-4">
        {userProfile && (
          <div className="rounded-md border bg-muted/50 p-3 mb-2 flex flex-col justify-center space-y-1">
             <div className="flex items-center justify-between text-xs font-medium text-muted-foreground mb-1">
                <span className="uppercase tracking-wider">{userProfile.tier} TIER</span>
                <span>{userProfile.unlockedSites.length} / {stringifiedLimit} Sites</span>
             </div>
             <div className="w-full h-1.5 rounded-full bg-secondary overflow-hidden">
                <div 
                  className="h-full bg-primary" 
                  style={{ width: tierLimit === 'Unlimited' ? '100%' : `${Math.min(100, Math.max(0, (userProfile.unlockedSites.length / (tierLimit as number)) * 100))}%` }} 
                />
             </div>
          </div>
        )}
