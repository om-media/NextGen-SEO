...
const blockedRules = [
  ...
  [/^PRODUCT\.md$/i, 'internal product context'],
  [/^PRODUCTION_CHECKLIST\.md$/i, 'internal production checklist'],
  [/^\.env(?!\.example$|\.production\.example$)/i, 'private environment configuration'],
  [/^\.factory\//i, 'local QA agent factory assets'],
  [/^\.server-dist\//i, 'generated server build output'],
  [/^(?:dist|build|coverage|\.tmp|\.npm-cache|uploads|playwright-report|test-results|blob-report)\//i, 'generated, cached, or local runtime output'],
  [/^droid-wiki\//i, 'generated/internal wiki export'],
  [/^docs\/.*(todo|competitor).*\.md$/i, 'internal planning or competitor notes'],
  [/^docs\/.*(?:plan|roadmap|evaluation|spec|stabilization-freeze).*\.md$/i, 'internal planning or stabilization notes'],
  [/^qa-.*\.(png|jpe?g|json|md)$/i, 'local QA evidence'],
  [/^qa-(?:artifacts?|results?)\//i, 'local QA evidence'],
  [/(^|\/)[^/]*\.log(\..*)?$/i, 'local log output'],
  [/(^|\/)[^/]*\.(db|sqlite)([-.].*)?$/i, 'local database file'],
  [/(^|\/)[^/]*\.(?:pem|key|p12|pfx)$/i, 'private key or certificate bundle'],
  [/(^|\/)(?:credentials[^/]*|service-account[^/]*)\.json$/i, 'service credential file'],
